...
function showWelcome() {
  ...
  let dist = getDistance(106.55, 29.57); //这里换成自己的经纬度
...
function createtime1() {
  var grt = new Date("05/25/2024 00:00:00"); //此处修改你的建站时间或者网站上线时间
...
function createtime2() {
  ...
  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ Powered by wyk🥝 %c 你正在访问wyk🥝の小家",
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| wyk🥝") ? title.substring(0, title.length - 14) : title;
    navigator.clipboard.writeText('wyk🥝的站内分享\n标题：' + subTitle + '\n链接：' + url + '\n欢迎来访！🍭🍭🍭');
    new Vue({
      data: function () {
        this.$notify({
          title: "成功复制分享信息🎉",
...
function createtime() {
  ...
  var start = new Date("05/25/2024 00:00:00"); // 旅行者1号开始计算的时间
  var dis = Math.trunc(23400000000 + ((now - start) / 1000) * 17); // 距离=秒数*速度 记住转换毫秒
  var unit = (dis / 149600000).toFixed(6);  // 天文单位
  // 网站诞生时间
  var grt = new Date("05/25/2024 00:00:00");
...
// 必应每日壁纸API
let bingDayBg = screen.width <= 768 ? "url(https://bing.img.run/m.php)" : "url(https://dailybing.com/)";
// 必应历史壁纸API
...
if (localStorage.getItem("blogbg") != undefined) {
  ...
  document.getElementById("defineBg").innerText = `:root{
    --default-bg: url(https://tuchuang.voooe.cn/images/2024/05/27/index.png);
    --darkmode-bg:url(https://tuchuang.voooe.cn/images/2024/05/27/index.png);
    --mobileday-bg: url(https://tuchuang.voooe.cn/images/2024/05/27/index.png);
    --mobilenight-bg: url(https://tuchuang.voooe.cn/images/2024/05/27/index.png);
  }`;
}
// 切换背景主函数
function changeBg(s) {
